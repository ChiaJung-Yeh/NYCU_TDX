...
Bus_Schedule=function(app_id, app_key, county, out=F){
  ...
  Sys.setlocale(category = "LC_ALL", locale = "cht")
  if (county=="Intercity"){
    url="https://ptx.transportdata.tw/MOTC/v2/Bus/Schedule/InterCity?&$format=XML"
  }else{
  ...
  if (substr(xml_text(x), 1, 4)=="City"){
    ...
    if (nrow(bus_info1)!=0){
      ...
      bus_schedule_1=data.frame(temp_id=c(1:length(xml_find_all(x, xpath = ".//d1:TripID"))))
      for (i in c("Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","StopSequence","StopUID","StopName//d1:Zh_tw","ArrivalTime","DepartureTime")){
        temp=xml_text(xml_find_all(x, xpath=paste0(".//d1:", i)))
        temp_id=grepl(ifelse(grepl("//", i), substr(i, 1, regexpr("//", i)-1), i), xml_find_all(x, xpath=paste0(".//d1:", ifelse(grepl("//", i), substr(i, 1, regexpr("//", i)-1), i))))
        temp_id=which(temp_id)
        temp=data.frame(temp_id, temp)
        colnames(temp)[2]=ifelse(grepl("//", i), substr(i, 1, regexpr("//", i)-1), i)
        bus_schedule_1=left_join(bus_schedule_1, temp, by="temp_id")
        cat(paste0("Attribute '", ifelse(grepl("//", i), substr(i, 1, regexpr("//", i)-1), i), "' is downloaded\n"))
      }
    ...
    if (nrow(bus_info2)!=0){
      ...
      bus_schedule_2=data.frame(temp_id=c(1:length(xml_find_all(x, xpath = ".//d1:TripID"))))
      for (i in c("StartTime","EndTime","MinHeadwayMins","MaxHeadwayMins","Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","StopSequence","StopUID","StopName//d1:Zh_tw","ArrivalTime","DepartureTime")){
        temp=xml_text(xml_find_all(x, xpath=paste0(".//d1:", i)))
        temp_id=grepl(ifelse(grepl("//", i), substr(i, 1, regexpr("//", i)-1), i), xml_find_all(x, xpath=paste0(".//d1:", ifelse(grepl("//", i), substr(i, 1, regexpr("//", i)-1), i))))
        temp_id=which(temp_id)
        temp=data.frame(temp_id, temp)
...
Bus_Schedule=function(app_id, app_key, county, out=F){
  ...
  if (substr(xml_text(x), 1, 4)=="City"){
    ...
    if (nrow(bus_info1)!=0){
      timetable=xml_length(xml_find_all(x, xpath = ".//d1:Timetables"))
      cat("#---Timetables Downloading---#\n")
      cat(paste0(length(timetable), " Routes\n"))

      bus_schedule_1=data.frame(temp_id=c(1:length(xml_find_all(x, xpath = ".//d1:TripID"))))

      temp_name=c("TripID","Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","StopSequence","StopUID","StopName","ArrivalTime","DepartureTime")
      temp_xml_name=c(".//d1:TripID",".//d1:Timetable//d1:ServiceDay//d1:Sunday",".//d1:Timetable//d1:ServiceDay//d1:Monday",".//d1:Timetable//d1:ServiceDay//d1:Tuesday",
                      ".//d1:Timetable//d1:ServiceDay//d1:Wednesday",".//d1:Timetable//d1:ServiceDay//d1:Thursday",".//d1:Timetable//d1:ServiceDay//d1:Friday",
                      ".//d1:Timetable//d1:ServiceDay//d1:Saturday",".//d1:StopSequence",".//d1:StopTimes//d1:StopTime//d1:StopUID",".//d1:StopTimes//d1:StopTime//d1:StopName//d1:Zh_tw",
                      ".//d1:ArrivalTime",".//d1:DepartureTime")

      for (i in c(1:length(temp_xml_name))){
        temp=xml_text(xml_find_all(x, xpath=temp_xml_name[i]))
        temp_id=grepl(temp_name[i], xml_find_all(x, xpath=".//d1:Timetable"))
        temp_id=which(temp_id)
        temp=data.frame(temp_id, temp)
        colnames(temp)[2]=temp_name[i]
        bus_schedule_1=left_join(bus_schedule_1, temp, by="temp_id")
        cat(paste0("Attribute '", temp_name[i], "' is downloaded\n"))
      }
      bus_schedule_1=select(bus_schedule_1, -temp_id)

      bus_info1=as.data.frame(lapply(bus_info1, rep, timetable))
      bus_schedule_timetable=cbind(bus_info1, bus_schedule_1)
    }
    ...
    if (nrow(bus_info2)!=0){
      ...
      bus_info2=as.data.frame(lapply(bus_info2, rep, freq))
